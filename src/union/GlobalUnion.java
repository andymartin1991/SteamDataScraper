...
public class GlobalUnion {
    ...
    public static void main(String[] args) {
        ...
        switch (opcion) {
            case 1:
                ...
                ejecutarFusion(
                    "data/exports/steam_games.json.gz",
                    "data/exports/rawg_games.json.gz",
                    "data/exports/global_games.json.gz",
                    "data/reports/conflicts_report.txt"
                );
                break;
            case 2:
                System.out.println("\n🌟 Iniciando Fusión de PRÓXIMOS LANZAMIENTOS...");
                ejecutarFusion(
                    "data/exports/steam_proximos_games.json.gz",
                    "data/exports/rawg_proximos_games.json.gz",
                    "app_data/global_proximos_games.json.gz",
                    "data/reports/conflicts_report_proximos.txt"
                );
                break;
            case 0:
        ...
        try {
            ...
            try (PrintWriter conflictWriter = new PrintWriter(new BufferedWriter(new FileWriter(reportFile)))) {
                ...
                try (InputStream is = new GZIPInputStream(new FileInputStream(rawgFile));
                    ...
                    // Soporte híbrido: Array o JSON Lines
                    if (parser.nextToken() == JsonToken.START_ARRAY) {
                        while (parser.nextToken() == JsonToken.START_OBJECT) {
                            procesarRawgGame(mapper.readTree(parser), steamGamesByTitle, rawgHuérfanos, mapper, conflictWriter);
                            totalRawg++;
                        }
                    } else if (parser.currentToken() == JsonToken.START_OBJECT) {
                        // Es JSON Lines o un solo objeto
                        do {
                            procesarRawgGame(mapper.readTree(parser), steamGamesByTitle, rawgHuérfanos, mapper, conflictWriter);
                            totalRawg++;
                        } while (parser.nextToken() == JsonToken.START_OBJECT);
                ...
                // Actualizar contador de fusiones exactas
                for (JsonNode node : steamGamesByTitle.values()) {
                    if (node.has("_merged")) mergedCount++;
                }
                // Ajuste porque mergedCount se cuenta diferente en el loop
                mergedCount = mergedCount - fusionesInteligentes; // Aproximación, mejor usar contadores en el loop
                ...
                // --- 4. Escribir Resultado Final (JSON LINES) ---
                System.out.println("   -> Escribiendo archivo final (JSON Lines): " + outputFile);
                
                try (Writer writer = new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(outputFile)), "UTF-8")) {
                    // NO escribimos '[' al inicio
                    ...
                    for (JsonNode game : steamGamesByTitle.values()) {
                        ...
                        // Escribir objeto en una línea + salto de línea
                        writer.write(mapper.writeValueAsString(game));
                        writer.write("\n");
                    }
                    ...
                    for (JsonNode rawgGame : rawgFinales) {
                        ...
                        writer.write(mapper.writeValueAsString(gameNode));
                        writer.write("\n");
                        rawgOnlyCount++;
                    }
                    // NO escribimos ']' al final
                }
                ...
                System.out.println("      - 🔗 Fusionados (Total):       " + (mergedCount + fusionesInteligentes)); // Corrección visual
                System.out.println("      - 🎮 Solo en RAWG:             " + rawgOnlyCount);
                System.out.println("      - 💻 Solo en Steam:            " + (steamGamesByTitle.size() - (mergedCount + fusionesInteligentes))); // Corrección visual
                System.out.println("   -----------------------------------------");
    ...
    private static void procesarRawgGame(JsonNode rawgGame, Map<String, JsonNode> steamGamesByTitle, List<JsonNode> rawgHuérfanos, ObjectMapper mapper, PrintWriter conflictWriter) {
